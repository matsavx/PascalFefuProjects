...
var
  ...
  str_in: String;
  info: arr;
  string_number_first, valid: Integer;
  valid_name_error, valid_gender_error, valid_date_error, valid_points_error: Boolean;
  ...
  begin
    ...
    begin
      ...
      case mounth of
        ...
        2: 
          if (day < 0) or ((day > 28) and (not(year mod 4 = 0))) then
          begin
            writeln('Ошибка в файле "first.txt" в строке ', string_number_first, ' (ошибка в поле "Дата рождения")');
            valid_date_error := true;
          end;
...
begin
  ...
  for var k := 1 to valid do
    println(info[k].name, info[k].gender, info[k].summ);
end.